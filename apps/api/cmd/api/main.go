...
func main() {
	...
	if err != nil {
		...
		log.Fatalf("configure stock count store: %v", err)
	}
	...
	batchCatalog, closeBatchCatalogStore, err := newRuntimeBatchCatalogStore(cfg, auditLogStore)
	if err != nil {
		if closeStockMovementStore != nil {
			if closeErr := closeStockMovementStore(); closeErr != nil {
				log.Printf("close stock movement store: %v", closeErr)
	...
	if err != nil {
		...
		log.Fatalf("configure warehouse receiving store: %v", err)
	}
	...
	if err != nil {
		...
		log.Fatalf("configure supplier rejection store: %v", err)
	}
	...
	if err != nil {
		...
		log.Fatalf("configure return receipt store: %v", err)
	}
	...
	registerSubcontractRoutes(routes, subcontractRouteHandlers{
		...
		subcontractFactoryClaims:           subcontractFactoryClaimsHandler(subcontractOrderService),
		subcontractFactoryClaimAcknowledge: subcontractFactoryClaimAcknowledgeHandler(subcontractOrderService),
		subcontractFactoryClaimResolve:     subcontractFactoryClaimResolveHandler(subcontractOrderService),
