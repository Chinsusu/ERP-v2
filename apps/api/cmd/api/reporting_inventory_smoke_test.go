...
func TestInventorySnapshotReportSmoke(t *testing.T) {
	...
	t.Run("JSON report returns inventory snapshot rows", func(t *testing.T) {
		req := smokeRequestAsRole(
			httptest.NewRequest(
				http.MethodGet,
				"/api/v1/reports/inventory-snapshot?business_date=2026-04-30&warehouse_id=wh-hcm&status=quarantine&expiry_warning_days=45",
				nil,
			),
			authConfig,
	...
	t.Run("CSV export returns stable inventory snapshot file", func(t *testing.T) {
		req := smokeRequestAsRole(
			httptest.NewRequest(
				http.MethodGet,
				"/api/v1/reports/inventory-snapshot/export.csv?business_date=2026-04-30&warehouse_id=wh-hcm&status=quarantine&expiry_warning_days=45",
				nil,
			),
			authConfig,
			auth.RoleWarehouseLead,
