...
| S16-03-01 | Material transfer PostgreSQL store | Material/packaging transfer list/save persists lines, source refs, issue evidence, and stock movement refs | `apps/api/internal/modules/production/application/subcontract_material_transfer_service.go` |
| S16-03-02 | Material transfer persistence tests | Fresh store reload and issue-materials lifecycle tests pass against PostgreSQL store | `apps/api/internal/modules/production/application/subcontract_material_transfer_service_test.go` |
| S16-04-01 | Sample approval PostgreSQL store | Sample submit/approve/reject evidence persists with reason and actor refs | `apps/api/internal/modules/production/application/subcontract_sample_approval_service.go` |
| S16-04-02 | Sample approval persistence tests | Fresh store reload and sample lifecycle tests pass against PostgreSQL store | `apps/api/internal/modules/production/application/subcontract_sample_approval_service_test.go` |
| S16-05-01 | Finished goods receipt PostgreSQL store | Receipt/accept/partial accept persists quantities, stock movement refs, and actor refs | `apps/api/internal/modules/production/application/subcontract_finished_goods_receipt_service.go` |
| S16-05-02 | Finished goods receipt persistence tests | Fresh store reload and receipt/accept lifecycle tests pass against PostgreSQL store | `apps/api/internal/modules/production/application/subcontract_finished_goods_receipt_service_test.go` |
